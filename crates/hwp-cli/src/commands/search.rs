...
impl SearchCommand {
    ...
    fn format_text(&self, matches: &[SearchMatch]) -> Result<String> {
        ...
        for match_item in matches {
            ...
            let highlighted = if self.regex {
                ...
                pattern.replace_all(&match_item.text, |caps: &regex::Captures| {
                    caps[0].red().bold().to_string()
                }).to_string()
            } else {
                match_item.text.replace(
                    &self.query,
                    &self.query.red().bold().to_string()
                )
            };
    ...
    fn format_json(&self, matches: &[SearchMatch]) -> Result<String> {
        let json_matches: Vec<_> = matches.iter().map(|m| {
            json!({
