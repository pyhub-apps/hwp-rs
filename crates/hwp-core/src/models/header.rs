...
impl HwpProperties {
    ...
    pub fn to_u32(&self) -> u32 {
        ...
        if self.is_distribution_document {
            value |= 0x0004;
        }
        if self.has_script {
            value |= 0x0008;
        }
        if self.is_drm_document {
            value |= 0x0010;
        }
        if self.has_xml_template_storage {
            value |= 0x0020;
        }
        if self.has_document_history {
            value |= 0x0040;
        }
        if self.has_certificate_signature {
            value |= 0x0080;
        }
        if self.certificate_encryption {
            value |= 0x0100;
        }
        if self.has_certificate_drm {
            value |= 0x0200;
        }
        if self.is_ccl_document {
            value |= 0x0400;
        }
        if self.is_mobile_optimized {
            value |= 0x0800;
        }
        if self.is_private_information_security {
            value |= 0x1000;
        }
        if self.has_change_tracking {
            value |= 0x2000;
        }
        if self.has_kogl_copyright {
            value |= 0x4000;
        }
        if self.has_video_control {
            value |= 0x8000;
        }
        if self.has_order_field_control {
            value |= 0x10000;
        }
