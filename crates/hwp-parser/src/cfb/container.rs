...
impl CfbContainer {
    ...
    pub fn read_stream<R: Read + Seek>(&mut self, reader: &mut R, name: &str) -> Result<&Stream> {
        ...
        let entry = self.directory.find(name)
            .ok_or_else(|| HwpError::InvalidFormat {
                reason: format!("Stream '{}' not found", name)
            })?;
        
        // Read the stream
        let stream = Stream::from_entry(reader, entry, &self.header, &self.fat, self.mini_fat.as_ref())?;
    ...
    pub fn read_stream_by_path<R: Read + Seek>(&mut self, reader: &mut R, path: &str) -> Result<&Stream> {
    ...
    pub fn has_stream(&self, name: &str) -> bool {
        self.directory.find(name).map(|e| e.is_stream()).unwrap_or(false)
    }
