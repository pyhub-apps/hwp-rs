...
impl Stream {
    ...
    pub fn is_compressed(&self) -> bool {
        ...
        if self.data.len() >= 16 {
            ...
            eprintln!("[DEBUG]   First {} bytes: {:02X?}", self.data.len(), &self.data);
        }
        ...
        if self.name == "DocInfo" || self.name.starts_with("BodyText/Section") {
            ...
            if self.data.len() >= 4 {
                ...
                let header = u32::from_le_bytes([header_bytes[0], header_bytes[1], header_bytes[2], header_bytes[3]]);
                ...
                let potential_size = u32::from_le_bytes([header_bytes[0], header_bytes[1], header_bytes[2], header_bytes[3]]);
                if potential_size > 0 && potential_size < (100 * 1024 * 1024) {
                    // Could be HWP compression format
                    eprintln!("[DEBUG]   -> Could be HWP compressed (size header: {})", potential_size);
                    return true;
                }
            }
        ...
        if self.data.len() >= 2 {
            ...
            if matches!(header, 0x789C | 0x78DA | 0x7801 | 0x785E | 0x78DE) {
                eprintln!("[DEBUG]   -> Zlib compression detected (header: 0x{:04X})", header);
                return true;
            }
        }
        ...
        if self.name == "DocInfo" || self.name.starts_with("BodyText/Section") {
            ...
            if self.data.len() >= 8 {
                ...
                if let Ok(result) = crate::compression::decompress_hwp(&self.data) {
                    eprintln!("[DEBUG] HWP decompression successful: {} bytes", result.len());
                    return Ok(result);
                }
            }
            ...
            if decoder.read_to_end(&mut decompressed).is_ok() {
                eprintln!("[DEBUG] Zlib decompression successful: {} bytes", decompressed.len());
                return Ok(decompressed);
            }
            
            return Err(HwpError::DecompressionError(
                format!("Failed to decompress {} stream with any method", self.name)
            ));
        }
