...
pub fn is_hwp_compressed(data: &[u8]) -> bool {
    ...
    if let Ok(uncompressed_size) = cursor.read_u32::<LittleEndian>() {
        ...
        uncompressed_size > 0 
            ...
            && uncompressed_size < (100 * 1024 * 1024) // Max 100MB reasonable limit
    } else {
        false
    }
}
...
pub fn decompress_hwp(data: &[u8]) -> Result<Vec<u8>> {
    if data.len() < 8 {
        return Err(HwpError::DecompressionError(
            format!("Data too small for HWP compression format: {} bytes", data.len())
        ));
    }
    ...
    eprintln!("  - Uncompressed size from header: {} bytes", uncompressed_size);
    ...
    if uncompressed_size > 100 * 1024 * 1024 {
        return Err(HwpError::DecompressionError(
            format!("Uncompressed size too large: {} bytes", uncompressed_size)
        ));
    }
    ...
    eprintln!("[DEBUG] First 8 bytes of compressed data: {:02X?}", &compressed_data[..8.min(compressed_data.len())]);
    
    // Decompress using raw deflate (windowBits = -15)
    match decompress_raw_with_size(compressed_data, uncompressed_size as usize) {
        Ok(result) => {
            eprintln!("[DEBUG] Decompression successful, {} bytes decompressed", result.len());
            Ok(result)
        }
        Err(e) => {
...
fn decompress_with_zlib_fallback(data: &[u8], expected_size: usize) -> Result<Vec<u8>> {
    ...
    match decoder.read_to_end(&mut decompressed) {
        Ok(_) => {
            eprintln!("[DEBUG] Zlib decompression successful (fallback), {} bytes", decompressed.len());
            Ok(decompressed)
        }
        Err(_) => {
            ...
            if data.len() > 4 {
                ...
                decoder.read_to_end(&mut decompressed)
                    .map_err(|e| HwpError::DecompressionError(
                        format!("All decompression methods failed: {}", e)
                    ))?;
                eprintln!("[DEBUG] Zlib decompression of data[4..] successful, {} bytes", decompressed.len());
                Ok(decompressed)
            } else {
...
pub fn decompress_raw_with_size(data: &[u8], expected_size: usize) -> Result<Vec<u8>> {
    ...
    match decompressor.decompress(data, &mut output, FlushDecompress::Finish) {
        ...
        Err(e) => {
            Err(HwpError::DecompressionError(
                format!("Raw deflate decompression failed: {}", e)
            ))
        }
    }
}
...
mod tests {
    ...
    fn test_hwp_decompression() {
        ...
        let mut hwp_data = Vec::new();
        hwp_data.write_u32::<LittleEndian>(original.len() as u32).unwrap();
