...
impl YamlFormatter {
    ...
    fn build_metadata(&self, doc_info: &DocInfo) -> Result<serde_json::Value> {
        ...
        if !doc_info.face_names.is_empty() {
            let fonts: Vec<serde_json::Value> = doc_info.face_names.iter().map(|face| {
        ...
        if !doc_info.styles.is_empty() {
            let styles: Vec<serde_json::Value> = doc_info.styles.iter().map(|style| {
                json!({
                    "name": style.name,
        ...
        if self.options.include_styles && !doc_info.char_shapes.is_empty() {
            let char_shapes: Vec<serde_json::Value> = doc_info.char_shapes.iter().take(5).map(|shape| {
                json!({
        ...
        if self.options.include_styles && !doc_info.para_shapes.is_empty() {
            let para_shapes: Vec<serde_json::Value> = doc_info.para_shapes.iter().take(5).map(|shape| {
                json!({
...
impl OutputFormatter for YamlFormatter {
    fn format_document(&self, document: &HwpDocument) -> Result<String> {
        ...
        // Format sections
        let sections: Vec<serde_json::Value> = document.sections.iter().enumerate().map(|(idx, section)| {
            let paragraphs: Vec<serde_json::Value> = section.paragraphs.iter().map(|para| {
                json!({
                    "text": para.text,
                })
            }).collect();
        ...
        let total_paragraphs: usize = document.sections.iter()
            ...
            .sum();
        let total_characters: usize = document.sections.iter()
        ...
        // Convert to YAML
        let yaml_string = serde_yaml::to_string(&doc_value)
            .map_err(|e| HwpError::InvalidFormat { reason: e.to_string() })?;
    ...
    fn format_section(&self, section: &Section, index: usize) -> Result<String> {
        ...
        serde_yaml::to_string(&section_value)
            .map_err(|e| HwpError::InvalidFormat { reason: e.to_string() })
    }
    ...
    fn format_paragraph(&self, paragraph: &Paragraph, index: usize) -> Result<String> {
        ...
        serde_yaml::to_string(&para_value)
            .map_err(|e| HwpError::InvalidFormat { reason: e.to_string() })
    }
}