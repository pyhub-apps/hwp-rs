...
fn test_docinfo_stream_compression_detection() {
    ...
    hwp_data.write_u32::<LittleEndian>(uncompressed.len() as u32).unwrap();
    hwp_data.extend_from_slice(&compressed_data);
    
    println!("HWP compressed DocInfo size: {} bytes", hwp_data.len());
    println!("First 16 bytes: {:02X?}", &hwp_data[..16.min(hwp_data.len())]);
    
    // Create a Stream object
    let stream = Stream::new("DocInfo".to_string(), hwp_data.clone());
    
    // Test compression detection
    assert!(stream.is_compressed(), "DocInfo stream should be detected as compressed");
...
fn test_bodytext_stream_compression() {
    ...
    // Test compression detection
    assert!(stream.is_compressed(), "BodyText stream should be detected as compressed");
...
fn test_problematic_docinfo_bytes() {
    ...
    assert!(stream.is_compressed(), "DocInfo should be assumed compressed");
...
fn test_mixed_compression_formats() {
    ...
    match stream1.decompress() {
        Ok(result) => {
            println!("Raw deflate decompression succeeded: {} bytes", result.len());
    ...
    let decompressed2 = stream2.decompress().expect("Zlib decompression should work");
