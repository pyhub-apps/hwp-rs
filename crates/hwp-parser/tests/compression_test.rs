...
use hwp_parser::compression::{decompress_hwp, decompress_raw, is_hwp_compressed};
use std::io::Write;
...
fn test_hwp_compression_with_real_like_data() {
    ...
    println!(
        "First 16 bytes (uncompressed): {:02X?}",
        &uncompressed_data[..16.min(uncompressed_data.len())]
    );
    ...
    println!(
        "First 16 bytes (compressed): {:02X?}",
        &compressed_data[..16.min(compressed_data.len())]
    );
    ...
    println!(
        "First 16 bytes: {:02X?}",
        &hwp_format_data[..16.min(hwp_format_data.len())]
    );

    // Test detection
    assert!(
        is_hwp_compressed(&hwp_format_data),
        "Should detect HWP compression"
    );
...
fn test_raw_deflate_without_header() {
    ...
    println!(
        "First 8 bytes of compressed: {:02X?}",
        &compressed[..8.min(compressed.len())]
    );
...
fn test_problematic_bytes() {
    ...
    println!(
        "That's {} MB - clearly wrong!",
        size_from_header / 1024 / 1024
    );
