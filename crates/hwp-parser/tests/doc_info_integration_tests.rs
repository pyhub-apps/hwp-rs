...
fn create_header(tag_id: u16, level: u8, size: usize) -> Vec<u8> {
    ...
    let value = (tag_id as u32) | ((level as u32) << 10) | ((size as u32) << 20);
...
fn create_sample_doc_info_data() -> Vec<u8> {
    ...
    let times_utf16: Vec<u8> = times_name
        .encode_utf16()
        .flat_map(|c| c.to_le_bytes())
        .collect();
...
fn create_sample_style_data() -> Vec<u8> {
    ...
    // Korean name: "바탕"
    let korean_name = "바탕";
    let korean_utf16: Vec<u8> = korean_name
        .encode_utf16()
        .flat_map(|c| c.to_le_bytes())
        .collect();
    ...
    let english_utf16: Vec<u8> = english_name
        .encode_utf16()
        .flat_map(|c| c.to_le_bytes())
        .collect();
